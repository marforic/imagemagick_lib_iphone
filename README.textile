...
<h1>Compiling ImageMagick and IMDelegates</h1>
...
<pre>./all.sh VERSION|clean</pre> 
where VERSION is the version of ImageMagick you want to compile (e.g.,
6.8.8-9), if 'clean' is passed, the script will clean all the log files and the
...
<pre>./env.sh</pre>
sets up environment variables used by the other scripts. This is most probably
the script you want to modify to suit your needs (e.g., set up which
architectures will be compiles, verbosity level, ...)
<pre>./flags.sh</pre>
